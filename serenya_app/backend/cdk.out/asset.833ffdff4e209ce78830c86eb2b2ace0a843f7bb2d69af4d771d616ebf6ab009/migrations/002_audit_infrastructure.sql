...
-- Enable pgcrypto extension for digest() function used in audit hashing
...
-- Note: Commented out because CURRENT_DATE in WHERE clause is not IMMUTABLE.
-- Instead, use a simple index on retention_expiry_date and filter in queries.
CREATE INDEX idx_audit_events_retention ON audit_events(retention_expiry_date);
