...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (event.fixUserFields) {
      ...
      // Drop and recreate the users table with larger fields
      const fixUsersSQL = `
        -- Drop existing users table and recreate with larger fields
        DROP TABLE IF EXISTS users CASCADE;
        
        -- Recreate users table with larger VARCHAR fields for encrypted data
        CREATE TABLE users (
          ...
          -- Unique constraints
          UNIQUE(provider_user_id, auth_provider),
          UNIQUE(email_hash)
        );
