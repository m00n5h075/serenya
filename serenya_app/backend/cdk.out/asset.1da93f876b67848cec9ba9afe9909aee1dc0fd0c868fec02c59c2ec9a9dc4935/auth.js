...
const { UserService, ConsentService, SessionService, DeviceService, SubscriptionService } = require('./database');
    ...
    if (!user) {
      ...
      try {
        ...
        const defaultSubscriptionData = {
          ...
          provider: 'manual', // Manual assignment for free tier
          externalSubscriptionId: `free-${user.id}-${Date.now()}`, // Unique ID for free subscription
          startDate: new Date().toISOString(),
          endDate: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString() // 1 year from now
        };
        
        const subscription = await SubscriptionService.createSubscription(defaultSubscriptionData);
        console.log('✅ Default subscription created successfully:', subscription.id);
        
        auditLog('default_subscription_created', user.id, {
          requestId,
          subscriptionId: subscription.id,
          subscriptionType: 'free'
        });
      } catch (subscriptionError) {
        console.error('⚠️ WARNING: Default subscription creation failed:', subscriptionError);
        console.error('Subscription error details:', subscriptionError.message, subscriptionError.stack);
        // Don't fail user creation if subscription creation fails - log and continue
        auditLog('default_subscription_failed', user.id, {
          requestId,
          error: sanitizeError(subscriptionError).substring(0, 100)
        });
      }
