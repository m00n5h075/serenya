...
DO $$ BEGIN
    CREATE TYPE chat_category_type AS ENUM ('explanation', 'doctor_prep', 'clarification', 'general', 'metrics', 'trends', 'stability', 'patterns');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
CREATE TABLE IF NOT EXISTS users (
    ...
    -- Profile information  
    email VARCHAR(1000) NOT NULL,       -- Encrypted email data (larger for encryption)
    email_hash VARCHAR(64),              -- SHA-256 hash of email for searchable encryption
    email_verified BOOLEAN DEFAULT FALSE,
    name VARCHAR(1000) NOT NULL,         -- Full display name (larger for encryption)
    given_name VARCHAR(1000),            -- First name (larger for encryption)
    family_name VARCHAR(1000),           -- Last name (larger for encryption)
...
CREATE TABLE IF NOT EXISTS user_sessions (
    ...
    -- Session context
    user_agent TEXT,                         -- Client user agent for session tracking
    source_ip INET,                          -- Source IP address for session tracking
...
('result', 'general', 'Lifestyle impact', 'Are there everyday habits that can influence [METRIC]? How might [METRIC] affect my daily health or long-term wellbeing?', 3, true, true),
...
('report', 'general', 'Lifestyle review', 'How could lifestyle changes have influenced these health trends over time? What lifestyle factors might explain the patterns in my data?', 5, false, true)
ON CONFLICT (content_type, category, display_order) DO UPDATE SET
    option_text = EXCLUDED.option_text,
    full_prompt_text = EXCLUDED.full_prompt_text,
    has_sub_options = EXCLUDED.has_sub_options,
    requires_health_data = EXCLUDED.requires_health_data,
...
DROP TRIGGER IF EXISTS update_biometric_registrations_updated_at ON biometric_registrations;
...
DROP TRIGGER IF EXISTS update_chat_options_updated_at ON chat_options;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'serenya_app') THEN
...
COMMENT ON COLUMN user_sessions.user_agent IS 'Client user agent string for session tracking and security analysis';
COMMENT ON COLUMN user_sessions.source_ip IS 'Source IP address for session tracking and security analysis';
...
CREATE INDEX IF NOT EXISTS idx_processing_jobs_retry ON processing_jobs(status, retry_count, scheduled_retry_at) WHERE status = 'retrying';
CREATE INDEX IF NOT EXISTS idx_processing_jobs_cleanup ON processing_jobs(ttl_expires_at) WHERE status IN ('completed', 'failed');
CREATE INDEX IF NOT EXISTS idx_processing_jobs_timeout ON processing_jobs(processing_started_at, status) WHERE status = 'processing';

-- Processing results indexes

-- Processing job events indexes  
CREATE INDEX IF NOT EXISTS idx_processing_job_events_job_id ON processing_job_events(job_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_processing_job_events_user_hash ON processing_job_events(user_id_hash, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_processing_job_events_type ON processing_job_events(event_type, event_status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_processing_job_events_correlation ON processing_job_events(correlation_id);

-- ========================================
-- 18. DOCUMENT PROCESSING TRIGGERS
-- ========================================

-- Function to set TTL expiry date for processing jobs
CREATE OR REPLACE FUNCTION set_processing_job_ttl()
RETURNS TRIGGER AS $$
BEGIN
...
-- Auto-set TTL for new processing jobs
DROP TRIGGER IF EXISTS set_processing_job_ttl_trigger ON processing_jobs;
CREATE TRIGGER set_processing_job_ttl_trigger
    BEFORE INSERT ON processing_jobs
    FOR EACH ROW EXECUTE FUNCTION set_processing_job_ttl();
...
DROP POLICY IF EXISTS processing_jobs_user_policy ON processing_jobs;
CREATE POLICY processing_jobs_user_policy ON processing_jobs
    USING (user_id = current_setting('app.current_user_id')::uuid);

DROP POLICY IF EXISTS processing_job_events_user_policy ON processing_job_events;
