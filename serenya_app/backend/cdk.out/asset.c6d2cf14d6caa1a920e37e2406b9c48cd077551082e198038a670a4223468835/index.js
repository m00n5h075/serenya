...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (!schemaExists) {
      ...
      console.log('CORE SCHEMA SQL LENGTH:', coreSchemaSQL.length);
      console.log('CORE SCHEMA SQL PREVIEW (first 500 chars):', coreSchemaSQL.substring(0, 500));
      
      // Split SQL into individual statements and execute them
      await executeMultiStatementSQL(client, coreSchemaSQL, 'Core Schema');
      ...
      await executeMultiStatementSQL(client, auditSchemaSQL, 'Audit Infrastructure');
...
/**
 * Execute multi-statement SQL by splitting on semicolons and running each statement individually
 */
async function executeMultiStatementSQL(client, sql, migrationName) {
  console.log(`Executing ${migrationName} migration...`);
  
  // Split SQL into individual statements
  // Remove comments and empty lines, split on semicolons
