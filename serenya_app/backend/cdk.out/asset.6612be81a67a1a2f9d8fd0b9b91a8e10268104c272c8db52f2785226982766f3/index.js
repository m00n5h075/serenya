...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (event.dropAndRecreate || event.force) {
      console.log('FORCE MODE: Dropping schema_versions table to force re-initialization...');
      try {
        await client.query('DROP TABLE IF EXISTS schema_versions CASCADE');
        console.log('schema_versions table dropped');
      } catch (error) {
        console.log('Error dropping schema_versions (might not exist):', error.message);
      }
    }
