...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // If createFreeSubscriptionForExistingUser parameter is passed, create a free subscription for the existing user
    if (event.createFreeSubscriptionForExistingUser) {
      console.log('CREATING FREE SUBSCRIPTION - Creating free subscription for existing user...');
      
      try {
        // Get the user ID (assuming only one user exists)
        ...
        const existingSubResult = await client.query('SELECT id FROM subscriptions WHERE user_id = $1', [userId]);
        ...
        if (existingSubResult.rows.length > 0) {
          ...
          return {
            ...
            body: JSON.stringify({
              ...
              userId,
              subscriptionCount: existingSubResult.rows.length,
              timestamp: new Date().toISOString()
            })
          };
        }
        ...
        const subscriptionResult = await client.query(`
          ...
          VALUES ($1, 'free', 'manual', $2, $3, $4, 'active')
          RETURNING id, subscription_type, subscription_status
        `, [
          userId,
        ...
        return {
          ...
          body: JSON.stringify({
            ...
            subscriptionType: subscriptionResult.rows[0].subscription_type,
            subscriptionStatus: subscriptionResult.rows[0].subscription_status,
            timestamp: new Date().toISOString()
          })
        };
